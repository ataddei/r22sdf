...
def Butterfly21 (i_control_s,i_data_aa,i_data_bb,o_data_aa,o_data_bb):
    selfc=Signal(complex(1,0))
    selfd=Signal(complex(1,0))
    ...
    def bf21():
        ...
        if i_control_s:
            ...
            o_data_aa.next=selfc
            o_data_bb.next=selfd
...
def Butterfly22 (i_control_t,i_control_s,i_data_aa,i_data_bb,o_data_aa,o_data_bb):
    selfa=Signal(complex(1,0))
    selfb=Signal(complex(1,0))
    selfc=Signal(complex(1,0))
    selfd=Signal(complex(1,0))
...
def stage(i_data,reset,clock,o_data,N=1):
    ...
    b=Signal(complex(1,0))
    c=Signal(complex(1,0))
    d=Signal(complex(1,0))
    control_s=Signal(False)
    control_t=Signal(False)
    counter=Signal(modbv(3,0,2**(N*2)))
